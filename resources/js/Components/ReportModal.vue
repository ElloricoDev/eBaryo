...
const emit = defineEmits(['close', 'submit', 'update:reason', 'update:description']);
    ...
    <div v-if="show" class="fixed inset-0 z-[9999] flex items-center justify-center">
      ...
      <div class="relative animate-fade-in bg-white bg-opacity-95 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden z-10 pointer-events-auto">
        ...
        <div class="bg-green-600 text-white px-4 py-3 rounded-t-2xl flex justify-between items-center">
          ...
          <button class="text-white focus:outline-none focus:ring-2 focus:ring-white rounded" @click="closeAll">&times;</button>
        </div>
        ...
        <div class="p-4">
          ...
          <div v-if="isMobile" class="relative mb-3">
            <div class="w-full border border-green-300 rounded shadow-sm px-3 py-2 bg-white focus:border-green-600 focus:ring-2 focus:ring-green-200 transition cursor-pointer" @click="handleInputClick" :aria-expanded="showDropdown" :tabindex="0">
              <span :class="{'text-gray-400': !localReason}">{{ localReason || 'Select a reason' }}</span>
              <span class="float-right">▼</span>
            </div>
            <ul v-if="showDropdown" ref="dropdownRef" class="absolute left-0 right-0 z-50 bg-white border border-green-300 rounded shadow-lg mt-1 max-h-60 overflow-auto">
              <li v-for="option in reasonOptions" :key="option" @click="handleOptionClick(option)" class="px-4 py-2 hover:bg-green-50 cursor-pointer" :class="{'bg-green-100 font-bold': localReason === option}">{{ option }}</li>
            </ul>
          </div>
          <select v-else v-model="localReason" class="w-full border border-green-300 rounded shadow-sm mb-3 focus:border-green-600 focus:ring-2 focus:ring-green-200 transition report-select">
          ...
          <textarea v-model="localDescription" rows="3" class="w-full border border-green-300 rounded shadow-sm focus:border-green-600 focus:ring-2 focus:ring-green-200 transition" placeholder="Any additional info..."></textarea>
          <div class="text-sm text-gray-500 mt-1">{{ localDescription.length }}/1000 characters</div>
        </div>
        <div class="px-4 py-3 border-t flex justify-end gap-2 bg-white bg-opacity-80">
          <button class="px-4 py-2 border rounded text-gray-600 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-green-400" @click="closeAll">
            <font-awesome-icon icon="xmark-circle" /> Cancel
          </button>
          <button class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 focus:bg-red-800 focus:outline-none focus:ring-2 focus:ring-red-400 disabled:opacity-50 flex items-center gap-2" @click="$emit('submit')" :disabled="loading || !localReason.trim()">
            <font-awesome-icon v-if="loading" icon="hourglass-half" />

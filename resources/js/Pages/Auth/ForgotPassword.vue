...
import { faEnvelope, faPaperPlane, faRightToBracket, faUserPlus } from '@fortawesome/free-solid-svg-icons'
    ...
    <div class="flex flex-col items-center justify-center">
        <div
            ...
            <h1 class="text-center text-green-700 text-2xl font-semibold mb-6">
                <font-awesome-icon icon="envelope" class="mr-2" /> Forgot Password
            </h1>

            <form @submit.prevent="form.post(route('password.email'))" class="space-y-5">
                <div class="relative">
                    <input v-model="form.email" type="email" id="email" required autofocus
                        ...
                        :class="{ 'border-red-500': form.errors.email }" placeholder="Email" aria-label="Email" />
                    <label for="email"
            ...
            <div class="text-center mt-6 space-y-2">
                <Link :href="route('login')" class="text-green-700 hover:underline block">
                <font-awesome-icon icon="right-to-bracket" /> Back to Login
                </Link>
                <Link :href="route('register')" class="text-green-700 hover:underline block">

...
import { faArrowLeft, faChevronLeft, faChevronRight, faSearchPlus, faSearchMinus, faExpand } from '@fortawesome/free-solid-svg-icons';
import * as pdfjsLib from 'pdfjs-dist';
import pdfjsWorkerSrc from 'pdfjs-dist/build/pdf.worker.mjs?url';

library.add(faArrowLeft, faChevronLeft, faChevronRight, faSearchPlus, faSearchMinus, faExpand);
...
const renderPDF = async (url) => {
  ...
  // Set start page based on lastPercent only on first load
  if (currentPage.value === 1 && lastPercent > 0 && lastPercent < 1) {
    currentPage.value = Math.max(1, Math.round(lastPercent * totalPages.value));
  ...
  if (isMobile()) {
    // Always fit to width at 100% zoom, allow zooming in beyond that
...
<template>
  ...
  <div class="fixed inset-0 bg-white z-50 flex flex-col">
    ...
    <div class="flex-1 min-h-0 pdf-viewer-container">
      <div class="pdf-controls card flex flex-wrap items-center justify-center gap-2 mb-4 mt-4 p-3 shadow-lg rounded-lg bg-white/90 backdrop-blur-md">
        <button @click="prevPage" :disabled="currentPage === 1 || isLoading" class="icon-btn" aria-label="Previous Page">
        ...
        <button @click="nextPage" :disabled="currentPage === totalPages || isLoading" class="icon-btn" aria-label="Next Page">
          <font-awesome-icon icon="chevron-right" />
        </button>
        <button @click="zoomOut" :disabled="zoom <= 0.5 || isLoading" class="icon-btn" aria-label="Zoom Out">
          <font-awesome-icon icon="search-minus" />
        </button>
        <span class="text-base">{{ (zoom * 100).toFixed(0) }}%</span>
        <button @click="zoomIn" :disabled="zoom >= 3 || isLoading" class="icon-btn" aria-label="Zoom In">
          <font-awesome-icon icon="search-plus" />
        </button>
        <button @click="fitToWidth" :disabled="isLoading" class="icon-btn fit-btn" aria-label="Fit to Width">
          <font-awesome-icon icon="expand" />
        </button>
      </div>
      <div v-if="isLoading" class="pdf-loading flex items-center justify-center w-full h-32">

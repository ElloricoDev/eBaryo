...
import { faArrowLeft, faPenToSquare, faUser, faPhone, faLocationDot, faEnvelope, faGraduationCap, faImage, faSave, faXmark } from '@fortawesome/free-solid-svg-icons';
library.add(faArrowLeft, faPenToSquare, faUser, faPhone, faLocationDot, faEnvelope, faGraduationCap, faImage, faSave, faXmark);
...
function onAvatarChange(e) {
  ...
  form.avatar = file
  avatarPreview.value = file ? URL.createObjectURL(file) : user.avatar || null
}
</script>
  ...
  <div class="relative min-h-screen max-w-5xl mx-auto px-2 sm:px-4 py-4 sm:py-6 overflow-x-hidden">
    ...
    <div class="animate-fade-in bg-white bg-opacity-90 rounded-2xl shadow-xl p-2 sm:p-6">
      <div class="flex items-center mb-6">
        <Link :href="route('user.profile.index')" class="text-green-700 hover:underline focus:outline-none focus:ring-2 focus:ring-green-400 flex items-center gap-2">
      ...
      <form @submit.prevent="submit" enctype="multipart/form-data" class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div class="col-span-full border-t border-green-100 my-2"></div>
          ...
          <!-- Student -->
          <div>
            <label class="text-green-700 font-medium block mb-1"><font-awesome-icon icon="graduation-cap" /> Student</label>
            <select v-model="form.student" class="w-full border border-green-300 rounded px-3 py-2 bg-transparent focus:border-green-600 focus:ring-2 focus:ring-green-200 transition">
          ...
          <!-- Avatar Upload -->
          <div>
            <label class="text-green-700 font-medium block mb-1"><font-awesome-icon icon="image" /> Avatar</label>
            <input
              type="file"
            ...
            <div v-if="avatarPreview" class="mt-3 flex justify-center">
              <img :src="avatarPreview" alt="Avatar Preview" class="w-20 h-20 rounded-full border-4 border-green-300 object-cover shadow-lg ring-2 ring-green-200" />
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center gap-3 mt-6">
          <button type="submit" class="bg-green-600 hover:bg-green-700 focus:bg-green-800 text-white px-5 py-2 rounded shadow font-semibold flex items-center gap-2 animate-pulse-cta focus:outline-none focus:ring-2 focus:ring-green-400 transition disabled:opacity-60 disabled:cursor-not-allowed" :disabled="form.processing">
